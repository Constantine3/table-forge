...
Match configuration accepts a `rolePreset` and never accepts an arbitrary role list. `percival-morgana` is the default for five, six, and seven players. `basic` remains available at every supported size. `mordred-oberon` is available only for seven players. The definition validates the preset against `playerCount`, resolves the complete deck, and permits `humanRole` only when that role occurs in the resolved deck. The `avalon/started` event stores the preset and complete assignment, and Avalon rules version `11` rejects earlier pre-release matches.
