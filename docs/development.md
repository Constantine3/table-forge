...
- pnpm 11.7.0 for dependency installation, pinned in `package.json`. Supported Node releases do not all include Corepack; use `npx --yes pnpm@11.7.0` when a global `pnpm` command is unavailable. After installation, use `npm run <script>` for root package scripts.
...
Setup is complete when `npm run typecheck` exits successfully.
...
`npm run hygiene` includes `publint`, which validates package entrypoints against the built `lib/*.js` files, and `verify-node-next-types`, which validates built declarations against a temporary NodeNext consumer. A fresh worktree has no bundled JS or declarations until `npm run build` runs; ordinary commits and pushes do not require that build unless their selected checks consume it.
...
- `pre-push` runs `npm run typecheck`, which completes the Host lib phase, including generated Typert contracts, before the Client TypeScript check without requiring a global pnpm executable.
...
Contributors can opt into the comprehensive local gate set with `npm run check:all`. The command is independent of the Git hooks and is not an agent instruction.
...
The root [contributor instructions](../AGENTS.md#commands) summarize common commands, while [`package.json`](../package.json) and [scripts/run-gates.ts](../scripts/run-gates.ts) own the current script and gate inventories. Select the smallest checks that cover the changed surface. Documentation changes use `npm run doc-sync`; package-public behavior changes also update the owning README or JSDoc, and built-artifact checks require `npm run build` first.
...
```sh
npm run dsh -- --profile headless "summarize this workspace"
```
