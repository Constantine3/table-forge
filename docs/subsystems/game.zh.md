...
游戏子系统运行确定性、事件溯源的对局，其中席位可以由人类或独立配置的 AI Agent 控制。交付产品包括支持人类对 AI 或 AI 对 AI 的剪刀石头布，以及由一名人类与其余 AI 组成或全部由 AI 控制的五至八人阿瓦隆。
...
游戏定义发布经部署配置解析后的设置 schema，验证配置与每个席位的当前动作、产生规则事件、将事件归约为状态、声明当前动作窗口并投影公开或席位范围的视图。每个窗口声明参与席位与提交状态是公开还是仅限必需行动的席位。规则定义不执行 I/O。Remote 目录会把每个设置 schema 与其规则版本一起返回，浏览器创建请求回传该版本，使过期页面在检查提供方或持久化之前被拒绝。引擎以幂等方式接受命令，以原子方式创建对局及其初始事件，在发布 `match/changed` 前追加后续事件批次，并从日志重建所有视图。定义和控制器注册后会恢复兼容的待处理 AI 动作。对局格式或游戏规则版本不受支持的记录会继续存储，但不会出现在产品列表和恢复中。
...
阿瓦隆设置会选择经过服务端校验的身份预设，并把预设与私有种子产生的分配一起持久化，以保持确定回放。默认预设组合派西维尔与莫甘娜，基础预设使用梅林、刺客、亚瑟的忠臣与爪牙，七人或八人进阶预设组合莫德雷德与奥伯伦。浏览器安全的 `@deepseek-ai/dsh-game-avalon-rules` 包向规则定义与设置界面提供同一份身份目录、牌组与任务规则；配置不接受任意身份拼装。队长公开队伍并选择顺时针或逆时针讨论；指定方向上的相邻席位先发言，所有非队长席位依次发言，队长进行最后一句归票发言并提交最终队伍。全部发言提交后，每个席位才收到不附带陈述的封存赞成票或否决票；通过后的队员提交封存任务动作。五人局的任务人数依次为 2、3、2、3、3，六人局为 2、3、4、3、4，七人局为 2、3、3、4、4，八人局为 3、4、4、5、5。五人局队伍获得三票即可通过，六人局与七人局需要四票，八人局需要五票。除七人局和八人局第四次任务需要两张失败票外，其他任务有一张失败票就会失败。三次任务失败或连续五支队伍被否决时邪方获胜；三次任务成功后，协作邪方会先在受限窗口中依次发言，再由刺客选择目标，只有最终裁定后才公开全部身份。
