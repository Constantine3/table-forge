...
This browser plugin contributes the `avalon` catalog card, five- through eight-seat setup, and round-table board to `dsh-client-ui-game`. Setup defaults to a six-player human-and-AI table and the balanced Percival-and-Morgana preset, lets the user switch to any Host-supported size or an AI-only table, and selects a provider independently for every AI seat. It derives the size options from the Remote configuration schema and compares the catalog rules version with the browser-safe Avalon rules package; a mismatch disables creation and asks the user to restart the service and refresh. Role setup is limited to the presets resolved by `@deepseek-ai/dsh-game-avalon-rules`; the Mordred-and-Oberon preset appears for seven and eight players, and changing to an unsupported size returns to the default preset. Setup and the active board display the actual role deck and counts. Each public role-count label reveals its faction and rule description on pointer hover or keyboard or touch focus; it never identifies a seat's hidden role. Human participation offers random assignment or a role that exists in the selected deck; AI-only setup removes that control and creates five through eight Agent seats.
