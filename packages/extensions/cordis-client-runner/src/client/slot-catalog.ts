...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'game.catalog.item',
    kind: 'list',
    scope: 'root',
    summary: 'Ordered setup choices contributed by installed game UI plugins.',
    doc: 'Ordered setup choices contributed by installed game UI plugins.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'root\' (client-ui-game), so it exists while that entry is mounted',
    occupants: [
      'client-ui-game-avalon AvalonCatalogItem id \'avalon\'',
      'client-ui-game-rps RpsCatalogItem id \'rps\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'game.catalog.item\', () => ctx.slots.register(\n      { name: \'game.catalog.item\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-game/src/client/GameApp.tsx:82',
  },
  {
    key: 'game.surface',
    kind: 'keyed',
    scope: 'root',
    summary: 'Game-specific setup and board selected by the durable game id.',
    doc: 'Game-specific setup and board selected by the durable game id.',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: avalon, rps',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'root\' (client-ui-game), so it exists while that entry is mounted',
    occupants: [
      'client-ui-game-avalon AvalonSurface key \'avalon\'',
      'client-ui-game-rps RpsSurface key \'rps\'',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'game.surface\', () => ctx.slots.register(\n      { name: \'game.surface\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-game/src/client/GameApp.tsx:84',
  },
  {
    key: 'root',
    kind: 'single',
