...
The model receives the configured player instruction, game rules, seat-scoped observation, active window id, and only `submit_game_action`. The shipped game profile requires explicit reasoning before every submission over the observation, prior rounds, score and objective, uncertainty and hidden information, opponent tendencies, candidate actions, risks, and likely counterplay without inventing unobserved information. It never receives raw match events, runtime workspace context, or another seat's Session.

#### Token effect

One Agent turn is requested per action, with at most `maxAttemptsPerAction - 1` retries after an idle turn without a valid submission.
